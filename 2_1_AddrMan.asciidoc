...
If the address position collides with an entry already filled in, the method `CAddrInfo::IsTerrible()` is used to decide if the old address will be evicted or not.
