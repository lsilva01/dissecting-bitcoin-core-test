...
The other criteria to check if the IBD is complete or not are:

1) verifying `CChain::Tip()`, which returns the index entry for the tip of the chain, or `nullptr` if none. 